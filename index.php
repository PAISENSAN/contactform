<?php
    define("FILE_DIR", "C:\Users\user\Pictures\contactfiles");
    ...
    if(!empty($clean['btn_confirm']) ){
		...
		$header .="From: 東京弁護士会 <colt.firearms.g@gmail.com>\n";
		$header .= "Reply-to: <colt.firearms.g@gmail.com>\n";
		...
		$auto_reply_text = "この度は、お問い合わせいただきありがとうございました。
		...
		if( !empty($clean['attachment_file']) ){
			...
			$body .= "Content-Transfer-Encording: base64\n";
	...
	if( !empty($clean['attachment_file']) ) {
		...
		mb_send_mail('krpo322@protonmail.com', $admin_reply_subject, $body, $header);
...
<form method="post" action="" enctype="multipart/form-data">
	...
	<div class="element_wrap">
		...
		<select name="age">
			...
			<option value="2"<?php if(!empty($clean['age']) && $clean['age'] === "2"){echo 'selected';} ?>>20~29歳</option>
			<option value="3"<?php if(!empty($clean['age']) && $clean['age'] === "3"){echo 'selected';} ?>>30~39歳</option>
			<option value="4"<?php if(!empty($clean['age']) && $clean['age'] === "4"){echo 'selected';} ?>>40~49歳</option>
			<option value="5"<?php if(!empty($clean['age']) && $clean['age'] === "5"){echo 'selected';} ?>>50~59歳</option>
	...
	<div class="element_wrap">
		<label for="agreement"><input id="agreement" type="checkbox" name="agreement" value="1" <?php if(!empty($clean['agreement']) && $clean['agreement'] === "1") {echo 'checked';} ?>>プライバシーポリシーに関する同意</label>
	</div>
